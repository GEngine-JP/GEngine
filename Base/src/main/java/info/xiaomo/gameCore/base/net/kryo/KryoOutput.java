...
/**
 * An OutputStream that buffers data in a byte array and optionally flushes to
...
public class KryoOutput extends OutputStream {
	...
	/**
	 * Sets a new OutputStream. The position and total are reset, discarding any
	 * buffered bytes.
	 * 
	 * @param outputStream
	 *            May be null.
	...
	/**
	 * Sets the buffer that will be written to. {@link #setBuffer(byte[], int)}
	 * is called with the specified buffer's length as the maxBufferSize.
	 */
	...
	/**
	 * Sets the buffer that will be written to. The position and total are
	 * reset, discarding any buffered bytes. The
	...
	/**
	 * Returns the buffer. The bytes between zero and {@link #position()} are
	 ...
	 * between zero and {@link #position()}.
	 */
	public byte[] toBytes() {
	...
	/**
	 * Returns the current position in the buffer. This is the number of bytes
	...
	/**
	 * Returns the total number of bytes written. This may include bytes that
	...
	/**
	 * Flushes any buffered bytes and closes the underlying OutputStream, if
	 * any.
	 */
	@Override
	 ...
	 * ASCII they are written more efficiently, else they are written as UTF8.
	 * If a string is known to be ASCII, {@link #writeAscii(String)} may be
	 * used. The string can be read using {@link KryoInput#readString()} or
	 * {@link KryoInput#readStringBuilder()}.
	 * 
	 * @param value
	 *            May be null.
	 */
	...
	/**
	 * Writes the length and CharSequence as UTF8, or null. The string can be
	 * read using {@link KryoInput#readString()} or
	 * {@link KryoInput#readStringBuilder()}.
	 * 
	 * @param value
	 *            May be null.
	 ...
	 * {@link KryoInput#readString()} or {@link KryoInput#readStringBuilder()}.
	 * 
	 * @param value
	 *            May be null.
	 */
	...
	private void writeUtf8Length(int value) {
		if (value >>> 6 == 0) {
			...
			require(2);
			byte[] buffer = this.buffer;
			buffer[position++] = (byte) (value | 0x40 | 0x80); // Set bit 7 and
																// 8.
	 ...
	 * @param optimizePositive
	 *            If true, small positive numbers will be more efficient (1
	 *            byte) and small negative numbers will be inefficient (9
	 *            bytes).
	 */
