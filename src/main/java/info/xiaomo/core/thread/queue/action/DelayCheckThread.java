package info.xiaomo.core.thread.queue.action;
...
public class DelayCheckThread extends Thread {
	...
	private final List<DelayGameAction> delayQqueue;    //延迟队列

	private final List<DelayGameAction> execQueue;    //执行队列,临时存储

	private volatile boolean isRunning;

	public DelayCheckThread(String prefix) {
		super(prefix + "DelayCheckThread");
		delayQqueue = new ArrayList<DelayGameAction>();
		execQueue = new ArrayList<DelayGameAction>();
		isRunning = true;
	...
	private int execActions() {
		int executeCount = 0;
		for (DelayGameAction delayAction : execQueue) {
			try {
	 ...
	 * @param delayAction
	 */
	public void addDelayAction(DelayGameAction delayAction) {
		synchronized (lock) {
