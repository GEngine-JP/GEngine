...
import static info.xiaomo.core.ai.btree.BehaviorTreeConstants.*;
...
import java.util.*;
import info.xiaomo.core.ai.btree.branch.*;
import info.xiaomo.core.ai.btree.decorator.Random;
import info.xiaomo.core.ai.btree.decorator.*;
import info.xiaomo.core.common.math.Vector3;
import info.xiaomo.core.common.utils.Args;
import info.xiaomo.core.common.utils.FileUtil;
import info.xiaomo.core.common.utils.ReflectUtil;
import info.xiaomo.core.common.utils.StringUtil;
...
public class BehaviorTreeManager {
		...
		try {
			if (!files.isEmpty()) {
				for (File file : files) {
					if (file.exists()) {
						// 加载行为树
						info.xiaomo.core.common.utils.Args.Two<String, BehaviorTree<? extends Person>> tree = createBehaviorTree(file);
	 ...
	 * @return
	 */
	private info.xiaomo.core.common.utils.Args.Two<String, BehaviorTree<? extends Person>> createBehaviorTree(File file) throws Exception {
	...
	private Task<Person> createTask(Element element) {
		...
		switch (element.getName()) {
			...
			case XML_PARALLEL:
				...
				if (Parallel.Policy.Selector.name().equalsIgnoreCase(policyAttr.getValue())) {
			...
			case XML_REPEAT:
				...
				if (timesAttr != null && !StringUtil.isBlank(timesAttr.getValue())) {
			...
			case XML_SEAMPHORE_GUARD:
				...
				if (nameAttr == null || StringUtil.isBlank(nameAttr.getValue())) {
					throw new IllegalStateException("信号量装饰器为设置name属性");
				}
			...
			case XML_RANDOM:
				...
				if (successAttr != null && !StringUtil.isBlank(successAttr.getValue())) {
	...
	private LeafTask<Person> createLeafTask(Element element) {
		...
		try {
			...
			for (Attribute attribute : (List<Attribute>) element.attributes()) {
				...
				if (StringUtil.isBlank(attribute.getValue())) {
