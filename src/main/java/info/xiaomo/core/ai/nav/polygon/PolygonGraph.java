...
import info.xiaomo.core.ai.nav.triangle.Triangle;
import info.xiaomo.core.ai.pfa.Connection;
import info.xiaomo.core.ai.pfa.IndexedGraph;
import info.xiaomo.core.ai.quadtree.QuadTree;
	...
	public void initPathRandomPoint() {
		...
		for (Polygon polygon : getPolygons()) {
			for (Vector3 point : polygon.randomPoints) {
				x = (int) point.x;
				z = (int) point.z;
				Map<Integer, List<Vector3>> map = allRandomPointsInPath.computeIfAbsent(x, k -> new HashMap<>());
