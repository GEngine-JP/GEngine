package info.xiaomo.gengine.config.excel.parser;

import static info.xiaomo.gengine.config.excel.parser.XmlElement.*;
...
public class DataConfigXmlParser {
    ...
    public static Map<String, TableDesc<IConfig>> parseConfigs(String xmlFilePath)
            throws Exception {
        Map<String, TableDesc<IConfig>> tableDescMap = new HashMap<>(10);
        ...
        if (configDataElement != null) {
            ...
            if (configsElement != null) {
                ...
                Iterator<Element> configElementIt = configsElement.elementIterator(CONFIG_ELEMENT);
                while (configElementIt.hasNext()) {
                    Element configElement = configElementIt.next();
                    ...
                    Class<IConfig> clz = (Class<IConfig>) Class.forName(className);
                    ...
                    String primaryKeysStr =
                            configElement.attributeValue(CONFIG_PRIMARY_KEYS_ATTRIBUTE);
                    ...
                    if (index != null && !index.trim().isEmpty()) {
                        try {
                            ...
                            throw new RuntimeException(
                                    String.format("【%s】对应配置的index【{}】错误", clz.getName(), index));
                        }
                    }
                    ...
                    if (header != null && !header.trim().isEmpty()) {
                        try {
                            ...
                            throw new RuntimeException(
                                    String.format("【%s】对应配置的header【{}】错误", clz.getName(), header));
                        }
                    }
                    ...
                    while (fieldElementIt.hasNext()) {
                        ...
                        if (fieldName == null || fieldName.trim().isEmpty()) {
                            throw new RuntimeException(
                                    String.format("【%s】field中name属性不能为空", clz.getName()));
                        }
                        ...
                        try {
                            ...
                            throw new RuntimeException(
                                    String.format("【%s】的属性字段【%s】不存在", clz.getName(), fieldName));
                        ...
                        String columnName =
                                fieldElement.attributeValue(FIELD_COLUMN_NAME_ATTRIBUTE);
                        ...
                        String convertersStr =
                                fieldElement.attributeValue(FIELD_CONVERTERS_ATTRIBUTE);
                        if (convertersStr != null && !convertersStr.trim().isEmpty()) {
                            String[] converters = convertersStr.split(",");
                            if (converters.length > 0) {
                                Class<? extends IConverter> converterClz =
                                        (Class<? extends IConverter>) Class.forName(converters[0]);
                                IConverter converter = converterClz.getConstructor().newInstance();
                                for (int i = 1; i < converters.length; i++) {
                                    converterClz =
                                            (Class<? extends IConverter>)
                                                    Class.forName(converters[i]);
                                    converter =
                                            converter.andThen(
                                                    converterClz.getConstructor().newInstance());
                                }
                        ...
                        List<Element> converters = fieldElement.elements(CONVERTER_ELEMENT);
                        if (converters != null && !converters.isEmpty()) {
                            Element converterElement = converters.get(0);
                            ...
                            if (converterClzName == null || converterClzName.trim().isEmpty()) {
                                converterClzName =
                                        converterElement.attributeValue(CONVERTER_CLASS_ATTRIBUTE);
                            }
                            if (converterClzName == null || converterClzName.trim().isEmpty()) {
                                throw new RuntimeException(
                                        String.format(
                                                "【%s】的属性字段【%s】存在一个空的converter配置",
                                                clz.getName(), fieldName));
                            }

                            Class<? extends IConverter> converterClz =
                                    (Class<? extends IConverter>) Class.forName(converterClzName);
                            IConverter converter = converterClz.getConstructor().newInstance();
                            for (int i = 1; i < converters.size(); i++) {
                                ...
                                if (converterClzName == null || converterClzName.trim().isEmpty()) {
                                    converterClzName =
                                            converterElement.attributeValue(
                                ...
                                converterClz =
                                        (Class<? extends IConverter>)
                                                Class.forName(converterClzName);
                                converter =
                                        converter.andThen(
                                                converterClz.getConstructor().newInstance());
                            }
                    ...
                    tableDescMap.put(clz.getName(), tableDesc);
                }
            }
        }
        return tableDescMap;
     ...
     */
    public static Map<String, Class<IConfig>> parseCaches(String xmlFilePath) throws Exception {
        Map<String, Class<IConfig>> caches = new HashMap<>(10);

        SAXReader reader = new SAXReader();
        Document document = reader.read(new File(xmlFilePath));
        Element root = document.getRootElement();
        // 获取configData节点
        Iterator<Element> rootIt = root.elementIterator(ROOT_ELEMENT);
        while (rootIt.hasNext()) {
            Element configDataElement = rootIt.next();
            // 获取configData的子节点caches
            Element cachesElement = configDataElement.element(CACHES_ELEMENT);
            if (cachesElement != null) {
                // 获取caches节点的子节点cache
                Iterator<Element> cacheElementIt = cachesElement.elementIterator(CACHE_ELEMENT);
                while (cacheElementIt.hasNext()) {
                    Element cacheElement = cacheElementIt.next();
