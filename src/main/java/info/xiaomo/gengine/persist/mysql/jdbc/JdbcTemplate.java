package info.xiaomo.gengine.persist.mysql.jdbc;

import java.sql.*;
import java.util.Date;
import java.util.*;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * JDBC模板
 *
 * @author 小莫
 */
public class JdbcTemplate {

    public static final RowMapper<Integer> INT_MAPPER = new IntegerRowMapper();
    public static final RowMapper<Long> LONG_MAPPER = new LongRowMapper();
    public static final RowMapper<String> STRING_MAPPER = new StringRowMapper();
    public static final RowMapper<Date> DATE_MAPPER = new DateRowMaper();
    public static final RowMapper<byte[]> BYTEARRAY_MAPPER = new ByteArrayRowMaper();
    public static final RowMapper<Map<String, Object>> MAP_MAPPER = new MapRowMapper();
    private static final Logger LOGGER = LoggerFactory.getLogger(JdbcTemplate.class);
    /** 数据库连接池 */
    private ConnectionPool pool;

    public JdbcTemplate(ConnectionPool pool) {
        this.pool = pool;
    }

    /**
     * 查询一条指定数据
     *
     * @param sql sql
     * @param mapper mapper
     * @param parameters parameters
     * @return T
     */
    public <T> T query(String sql, RowMapper<T> mapper, Object... parameters) {

        Connection conn = null;
        PreparedStatement pStatement = null;
        ResultSet rs = null;

        try {
            conn = pool.getConnection();
            pStatement = conn.prepareStatement(sql);
            for (int i = 0; i < parameters.length; i++) {
                pStatement.setObject(i + 1, parameters[i]);
            }
            rs = pStatement.executeQuery();
            if (rs.next()) {
                return mapper.mapping(rs);
            }
        } catch (Exception e) {
            LOGGER.error("查询单条数据失败,sql:" + sql, e);
        } finally {
            release(conn, pStatement, rs, mapper);
        }
        return null;
    }

    public <T> List<T> queryList(String sql, RowMapper<T> mapper, Object... parameters) {

        Connection conn = null;
        PreparedStatement pStatement = null;
        ResultSet rs = null;
        try {
            conn = pool.getConnection();
            pStatement = conn.prepareStatement(sql);
            for (int i = 0; i < parameters.length; i++) {
                pStatement.setObject(i + 1, parameters[i]);
            }
            rs = pStatement.executeQuery();
            List<T> ret = new ArrayList<>();
            while (rs.next()) {
                ret.add(mapper.mapping(rs));
            }
            return ret;
        } catch (Exception e) {
            LOGGER.error("查询多条数据失败,sql:" + sql, e);
        } finally {
            release(conn, pStatement, rs, mapper);
        }
        return Collections.emptyList();
    }

    public int update(String sql, Object... parameters) {

        Connection conn = null;
        PreparedStatement pStatement = null;
        try {
            conn = pool.getConnection();
            pStatement = conn.prepareStatement(sql);
            for (int i = 0; i < parameters.length; i++) {
                pStatement.setObject(i + 1, parameters[i]);
            }
            return pStatement.executeUpdate();
        } catch (Exception e) {
            LOGGER.error("数据库更新失败,sql:" + sql, e);
            return -1;
        } finally {
            release(conn, pStatement, null, null);
        }
    }

    public void createTable(String sql) {

        Connection conn = null;
        PreparedStatement pStatement = null;
        try {
            conn = pool.getConnection();
            pStatement = conn.prepareStatement(sql);
            pStatement.execute(sql);
        } catch (Exception e) {
            LOGGER.error("数据库更新失败,sql:" + sql, e);
        } finally {
            release(conn, pStatement, null, null);
        }
    }

    public void batchUpdate(String sql, List<Object[]> parameters) {
        PreparedStatement pStatement = null;
        Connection conn = null;
        try {
            conn = pool.getConnection();
            pStatement = conn.prepareStatement(sql);
            for (Object[] parameterArray : parameters) {
                for (int i = 0; i < parameterArray.length; i++) {
                    pStatement.setObject(i + 1, parameterArray[i]);
                }
                pStatement.addBatch();
            }
            pStatement.executeBatch();
        } catch (Exception e) {
            LOGGER.error("批处理更新失败,sql:" + sql, e);
        } finally {
            release(conn, pStatement, null, null);
        }
    }

    private void release(
            Connection conn, PreparedStatement pStatement, ResultSet rs, RowMapper<?> mapper) {

        if (rs != null) {
            try {
                rs.close();
            } catch (SQLException e) {
                LOGGER.error("Result关闭出错。", e);
            }
        }

        if (pStatement != null) {
            try {
                pStatement.close();
            } catch (SQLException e) {
                LOGGER.error("PreparedStatement关闭出错。", e);
            }
        }

        if (conn != null) {
            try {
                pool.release(conn);
            } catch (SQLException e) {
                LOGGER.error("Connection关闭出错。", e);
            }
        }

        if (mapper != null) {
            try {
                mapper.release();
            } catch (Exception e) {
                LOGGER.error("Mapper释放出错。", e);
            }
        }
    }

    public ConnectionPool getPool() {
        return pool;
    }

    public void setPool(ConnectionPool pool) {
        this.pool = pool;
    }

    public boolean hasTable(String tableName) throws SQLException {
        boolean result = false;
        Connection connection = pool.getConnection();
        DatabaseMetaData metaData = connection.getMetaData();
        ResultSet tables = metaData.getTables(null, null, tableName, null);
        if (tables.next()) {
            result = true;
        }
        return result;
    }

    private static class IntegerRowMapper implements RowMapper<Integer> {

        @Override
        public Integer mapping(ResultSet rs) throws SQLException {
            return rs.getInt(1);
        }

        @Override
        public void release() {}
    }

    private static class LongRowMapper implements RowMapper<Long> {

        @Override
        public Long mapping(ResultSet rs) throws SQLException {
            return rs.getLong(1);
        }

        @Override
        public void release() {}
    }

    private static class StringRowMapper implements RowMapper<String> {

        @Override
        public String mapping(ResultSet rs) throws SQLException {
            return rs.getString(1);
        }

        @Override
        public void release() {}
    }

    private static class DateRowMaper implements RowMapper<Date> {

        @Override
        public Date mapping(ResultSet rs) throws SQLException {
            return rs.getDate(1);
        }

        @Override
        public void release() {}
    }

    private static class ByteArrayRowMaper implements RowMapper<byte[]> {

        @Override
        public byte[] mapping(ResultSet rs) throws SQLException {
            return rs.getBytes(1);
        }

        @Override
        public void release() {}
    }

    private static class MapRowMapper implements RowMapper<Map<String, Object>> {

        private final ThreadLocal<ResultSetMetaData> threadLocal = new ThreadLocal<>();

        @Override
        public Map<String, Object> mapping(ResultSet rs) throws SQLException {
            Map<String, Object> ret = new HashMap<>(10);
            ResultSetMetaData rsmd = threadLocal.get();
            if (rsmd == null) {
                rsmd = rs.getMetaData();
                threadLocal.set(rsmd);
            }
            int column = rsmd.getColumnCount();
            for (int i = 1; i <= column; i++) {
                ret.put(rsmd.getColumnName(i), rs.getObject(i));
            }
            return ret;
        }

        @Override
        public void release() {
            threadLocal.remove();
        }
    }
}
