...
public class PersistTask extends Thread {
    ...
    /** 需要持久化的id列表 */
    private final Map<Long, PersistType> persistMap = new HashMap<>(10);

    /** 需要插入的数据的参数（每次run方法调用的时候使用，使用完毕clear） */
    private final LinkedList<Object[]> insertParams = new LinkedList<>();

    /** 需要更新的数据的参数（每次run方法调用的时候使用，使用完毕clear） */
    private final LinkedList<Object[]> updateParams = new LinkedList<>();

    /** 需要删除的数据的参数（每次run方法调用的时候使用，使用完毕clear） */
    private final LinkedList<Object[]> deleteParams = new LinkedList<>();

    /** 上一次插入失败的参数 */
    private final List<Object[]> insertErrorParams = new LinkedList<>();

    /** 上一次更新失败的参数 */
    private final List<Object[]> updateErrorParams = new LinkedList<>();

    /** 上一次删除失败的参数 */
    private final List<Object[]> deleteErrorParams = new LinkedList<>();
    ...
    private final PersistAbleCache cache;

    private final PersistFactory persistFactory;

    public PersistTask(
            JdbcTemplate template, PersistFactory persistFactory, PersistAbleCache cache) {
    ...
    public void run() {
        try {
            ...
            cloneMap.entrySet().stream()
                    .filter(
                            longPersistTypeEntry -> {
