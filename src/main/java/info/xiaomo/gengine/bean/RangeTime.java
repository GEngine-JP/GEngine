...
public class RangeTime {
    ...
    private final RangeReg[] year;
    private final RangeReg[] month;
    private final RangeReg[] day;
    private final RangeReg[] week;
    private final RangeReg[] second;
    private final transient String[] times; // 配置的时间字符串
    ...
    public long getDataEndTime(Calendar calendar) {
        ...
        try {
            ...
            calendar1.set(
                    ...
                    m - 1,
                    d,
                    getDayOfHour(endSeconds),
                    getHourOfMinute(endSeconds),
                    getMinuteOfSecond(endSeconds));
    ...
    public String checkConfigTimeStr() {
        ...
        for (int i = 0; i < year.length; i++) {
            ...
            if (yearStr != null) {
                // 月
                String monthStr;
                if ((monthStr = getDateStr(calendar.get(Calendar.MONTH) + 1, month[i].getRange()))
                        != null) {
                    // 星期
                    ...
                    if ((weekStr = getDateStr(w, week[i].getRange())) != null) {
                        // 日
                        String dayStr;
                        if ((dayStr = getDateStr(calendar.get(Calendar.DATE), day[i].getRange()))
                                != null) {
                            // 时间
                            String timeStr;
            ...
            for (String item : items) {
                ...
                if (date.get(Calendar.HOUR_OF_DAY) > hour // 小时
                        || (date.get(Calendar.HOUR_OF_DAY) == hour
                                && date.get(Calendar.MINUTE) > minute) // 分钟
                        || (minute == date.get(Calendar.MINUTE)
                                && date.get(Calendar.SECOND) >= s)) // 秒
                {
