...
@Data
public class BaseRole extends AbsPerson {
	private static final transient Logger LOGGER = LoggerFactory.getLogger(BaseRole.class);

	/**
	 * setter 方法集合
	 */
	@JSONField(serialize = false)
	protected static final transient Map<String, Method> WRITEMETHODS = ReflectUtil.getReadMethod(BaseRole.class);
	...
	public BaseItem getItem(long itemId) {
		String key = HallKey.Role_Map_Packet.getKey(id);
		return JedisManager.getInstance().hget(key, itemId, BaseItem.class);
	}
	 ...
	 * @return
	 */
	public Map<Long, BaseItem> getItems() {
		String key = HallKey.Role_Map_Packet.getKey(id);
		return JedisManager.getInstance().hgetAll(key, Long.class, BaseItem.class);
	}
