...
public class DefaultStateMachine<E, S extends State<E>> implements StateMachine<E, S> {
    ...
    /**
     * Creates a {@code DefaultStateMachine} for the specified owner, initial state and global
     * state.
     *
     * @param owner the owner of the state machine
     * @param initialState the initial state
    ...
    /**
     * 更新全局状态和当前状态<br>
     * Updates the state machine by invoking first the {@code execute} method of the global state
     * (if any) then the {@code execute} method of the current state.
     */
    @Override
    public void update() {
     ...
     *
     * <p>This implementation assumes states are singletons (typically an enum) so they are compared
     * with the {@code ==} operator instead of the {@code equals} method.
     *
    ...
    /**
     * Handles received telegrams. The telegram is first routed to the current state. If the current
     * state does not deal with the message, it's routed to the global state's message handler.
     *
