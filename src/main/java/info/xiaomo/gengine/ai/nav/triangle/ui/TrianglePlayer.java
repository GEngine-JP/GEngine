...
import info.xiaomo.gengine.ai.nav.triangle.TrianglePointPath;
...
public class TrianglePlayer {
    ...
    public void update(double seconds) {
        ...
        for (int i = 0; i < paths.size(); i++) {
            ...
            oldPos.x = pos.x;
            oldPos.z = pos.z;
            double distUntilTargetReached =
                    Vector3.dst(currentTargetPoint.x, currentTargetPoint.z, pos.x, pos.z);
            ...
            if (xCoordToWorkOutAngle != 0 || yCoordToWorkOutAngle != 0) {
                double dirAngle =
                        Vector3.findAngle(
                                0,
                                0,
                                xCoordToWorkOutAngle,
                                yCoordToWorkOutAngle); // (float)Math.atan(yCoordToWorkOutAngle/xCoordToWorkOutAngle);
