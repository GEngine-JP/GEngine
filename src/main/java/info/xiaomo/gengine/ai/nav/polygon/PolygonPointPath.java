...
public class PolygonPointPath implements Iterable<Vector3> {
    ...
    public void calculateForGraphPath(PolygonGraphPath polygonlePath, boolean calculateCrossPoint) {
        ...
        // 起始坐标从上向下的射线
        //		if (!Intersector.intersectRayTriangle(ray, startPolygon.a, startPolygon.b,
        // startPolygon.c, null)) {
        ...
        //			float dst;
        //			// A-B
        //			if ((dst = GeometryUtil.nearestSegmentPointSquareDistance(projection, startPolygon.a,
        // startPolygon.b,
        //					start)) < minDst) {
        //				minDst = dst;
        ...
        //			}
        //			// B-C
        //			if ((dst = GeometryUtil.nearestSegmentPointSquareDistance(projection, startPolygon.b,
        // startPolygon.c,
        //					start)) < minDst) {
        //				minDst = dst;
        //				newStart.set(projection);
        //			}
        //			// C-A
        //			if ((dst = GeometryUtil.nearestSegmentPointSquareDistance(projection, startPolygon.c,
        // startPolygon.a,
        //					start)) < minDst) {
        //				minDst = dst;
        //				newStart.set(projection);
        ...
        if (nodes.size() == 0) { // 起点终点在同一三角形中
            ...
            lastEdge =
                    new PolygonEdge(
                            nodes.get(nodes.size() - 1).getToNode(),
                            nodes.get(nodes.size() - 1).getToNode(),
                            end,
                            end);
    ...
    /**
     * @param index
     * @return @ 获取寻路向量点 A path point which crosses one or more edges in the navigation mesh.
     */
     ...
     * 根据多边形路径计算最短路径坐标点 <br>
     *
     * <p>1.下一组边与上一个路径点的向量全部在前一组边的范围内，直接把下一组边的两点替换前一组边的两点。<br>
     ...
     * http://blog.csdn.net/yxriyin/article/details/39207709 Calculate the shortest point path
     * through the path triangles, using the Simple Stupid Funnel Algorithm.
     *
     * @return
     */
        ...
        for (int i = 1; i < numEdges(); ++i) {
                ...
                if (leftPlaneRightDP != Plane.PlaneSide.Front) {
                    ...
                    if (calculateCrossPoint) {
                        calculateEdgeCrossings(
                                lastRestart, leftIndex, funnel.pivot, funnel.leftPortal);
                    } else {
            ...
            if (leftPlaneLeftDP != Plane.PlaneSide.Front) {
                ...
                if (rightPlaneLeftDP != Plane.PlaneSide.Front) {
                    ...
                    if (calculateCrossPoint) {
                        calculateEdgeCrossings(
                                lastRestart, rightIndex, funnel.pivot, funnel.rightPortal);
                    } else {
    ...
    /**
     * 计算存储和边的交叉点<br>
     * Store all edge crossing points between the start and end indices. If the path crosses exactly
     * the start or end points (which is quite likely), store the edges in order of crossing in the
     * EdgePoint data structure.
     *
     * <p>Edge crossings are calculated as intersections with the plane from the start, end and up
     * vectors.
     *
     * @param startIndex
     * @param endIndex
     ...
     * @param endPoint
     */
    private void calculateEdgeCrossings(
            int startIndex, int endIndex, Vector3 startPoint, Vector3 endPoint) {
        ...
        for (int i = startIndex; i < endIndex; i++) {
            ...
            } else if (Intersector.intersectSegmentPlane(
                            edge.leftVertex, edge.rightVertex, crossingPlane, tmp1)
