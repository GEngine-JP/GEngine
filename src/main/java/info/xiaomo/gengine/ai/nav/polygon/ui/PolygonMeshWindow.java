...
public class PolygonMeshWindow {
    ...
    public PolygonMeshWindow(PolygonNavStart main) {
        ...
        polygonViewPane.addMouseListener(
                new MouseListener() {
        ...
        polygonViewPane.addMouseMotionListener(
    ...
    public PolygonNavMesh loadMap(String filePath, int scale) {
        ...
        try {
            ...
            if (map != null && player != null) {
                ...
                polygonViewPane.setPreferredSize(
                        new Dimension(
                                (int)
                                        (navMeshData.getWidth()
                                                * scale
                                                * (1
                                                        + navMeshData.getHeight()
                                                                / (navMeshData.getWidth()
                                                                        + navMeshData
                                                                                .getHeight()))),
                                (int)
    ...
    public void randMove() {
        Vector3 t =
                polygonViewPane
                        .getMap()
                        .getRandomPointInPath(polygonViewPane.getPlayer().pos, 800, 2);
        if (t != null) {
                ...
                if (awtEvent instanceof MouseEvent) {
                    ...
                    switch (e.getID()) {
                        ...
                        case MouseEvent.MOUSE_CLICKED:
                            /* 有缩放 */
                            Vector3 positon =
                                    this.player.getMap().getPointInPath(e.getX(), e.getY());
                            if (positon == null) {
                            ...
                            if (e.getClickCount() > 1) {
                                Polygon polygon =
                                        polygonViewPane
                                                .getPlayer()
                                                .getMap()
                                                .getPolygon(new Vector3(e.getX(), e.getY()));
                                if (polygon != null) {
    ...
    public void setLocationVector(float x, float z) {
        ...
        this.main.showLog(
                String.format("坐标点%s 在行走层%s", pos.toString(), String.valueOf(pointInPath)));
