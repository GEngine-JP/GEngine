...
public class Matrix4 implements Serializable {
    ...
    /**
     * Multiplies the matrix mata with matrix matb, storing the result in mata. The arrays are
     * assumed to hold 4x4 column major matrices as you can get from {@link Matrix4#val}. This is
     * the same as {@link Matrix4#mul(Matrix4)}.
     *
     ...
     * column major matrix as you can get from {@link Matrix4#val}. The vectors array is assumed to
     * hold 3-component vectors. Offset specifies the offset into the array where the x-component of
     * the first vector is located. The numVecs parameter specifies the number of vectors stored in
     * the vectors array. The stride parameter specifies the number of floats between subsequent
     * vectors and must be >= 3. This is the same as {@link Vector3#mul(Matrix4)} applied to
     * multiple vectors.
     *
     * @param mat the matrix
    ...
    /**
     * Multiplies the vectors with the given matrix, , performing a division by w. The matrix array
     ...
     * array where the x-component of the first vector is located. The numVecs parameter specifies
     * the number of vectors stored in the vectors array. The stride parameter specifies the number
     * of floats between subsequent vectors and must be >= 3. This is the
     *
     * @param mat the matrix
     * @param vecs the vectors
     * @param offset the offset into the vectors array
    ...
    /**
     * Multiplies the vector with the top most 3x3 sub-matrix of the given matrix. The matrix array
     * is assumed to hold a 4x4 column major matrix as you can get from {@link Matrix4#val}. The
     * vector array is assumed to hold a 3-component vector, with x being the first element, y being
     * the second and z being the last component. The result is stored in the vector array. This is
     * the same as
     *
     * @param mat the matrix
     * @param vec the vector.
     ...
     *     in <a href="http://en.wikipedia.org/wiki/Row-major_order">column major</a> order.
     * @return This matrix for the purpose of chaining methods together.
     */
    ...
    /**
     * Averages the given transforms and stores the result in this matrix. Translations and scales
     * are lerped while rotations are slerped. Does not destroy the data contained in t.
     *
    ...
    /**
     * Sets this matrix to the given 3x3 matrix. The third column of this matrix is set to
     * (0,0,1,0).
     *
     * @param mat the matrix
     */
    ...
    /**
     * Assumes that both matrices are 2D affine transformations, copying only the relevant
     * components. The copied values are:
     *
     * <pre>
     *      [  M00  M01   _   M03  ]
     *      [  M10  M11   _   M13  ]
    ...
    public float getScaleX() {
        return (MathUtil.isZero(val[Matrix4.M01]) && MathUtil.isZero(val[Matrix4.M02]))
    ...
    public float getScaleY() {
        return (MathUtil.isZero(val[Matrix4.M10]) && MathUtil.isZero(val[Matrix4.M12]))
    ...
    public float getScaleZ() {
        return (MathUtil.isZero(val[Matrix4.M20]) && MathUtil.isZero(val[Matrix4.M21]))
    ...
    /**
     * Copies the 4x3 upper-left sub-matrix into float array. The destination array is supposed to
    ...
    public boolean hasRotationOrScaling() {
        return !(MathUtil.isEqual(val[M00], 1)
                && MathUtil.isEqual(val[M11], 1)
                && MathUtil.isEqual(val[M22], 1)
                && MathUtil.isZero(val[M01])
                && MathUtil.isZero(val[M02])
                && MathUtil.isZero(val[M10])
                && MathUtil.isZero(val[M12])
                && MathUtil.isZero(val[M20])
                && MathUtil.isZero(val[M21]));
    }
}
